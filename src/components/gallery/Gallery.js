...
function mapItemData(item, type = 'fauna') {
  ...
  const itemId = type === 'fauna' ? item.id_animal : item.id_planta;
  const routePrefix = type === 'fauna' ? 'fauna' : 'flora';

  // Imagen: si existe url_imagen usarla, si no usar placeholder
  const imageUrl = item.url_imagen || item.foto_principal?.url_foto || '/placeholder.jpg';
...
export function renderGallery(items = [], { size = 'md', glass = false, type = 'fauna' } = {}) {
  ...
  // Mapear y renderizar tarjetas
  const cardsHTML = items
    .map(item => {
      try {
        const mappedData = mapItemData(item, type);
        return renderCardFn(mappedData, { size, glass });
      } catch (error) {
  ...
  container.innerHTML = `<div class="grid grid-3">${cardsHTML}</div>`;
  return container;
}
